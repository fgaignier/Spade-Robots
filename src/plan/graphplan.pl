...
%    WARNING, we have modified the program from its original version to correct major bugs
%    ###########################################################
...
find_plan(N, CurrentState, InitialState, PrevActions, Plan):-
	...
	% Fabrice Gaignier modification. Just need one plan, and not all possible combinaisons
	N1 is N-1, !,  
...
%% Generate shorter plans using the following strategy
%% Be lazy: prefer no ops
choose_action_to_achieve_pred(N, Pred, no_op(X)):-  plan_graph(N, add, Pred, no_op(X)).                   

 %% Choose real actions only if no ops fail
choose_action_to_achieve_pred(N, Pred, OtherAction):-   
...
mutex_add_add_conflict(N):-
	...
	not( (
	      ...
	      %% Action11 \= Action22, Fabrice Gaignier modification: the same action can generate both Pred1 and Pred2
