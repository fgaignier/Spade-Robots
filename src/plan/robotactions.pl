...
/* Starting situation */
/* getCoffee() */
getCoffee(P):- plan([at(box1, box), at(box2, tv), at(samira, start), at(raphael, tv), handempty(samira), on(cup, box1), free(box2), accessible(box)], [at(samira, tv), on(cup, box2)], robot, P).

/* clear box1 */
clear(P):- plan([at(box1, box), at(box2, tv), at(samira, start), at(raphael, tv), handempty(samira), handempty(raphael), on(cup, box1), free(box3), accessible(box)], [holding(samira, cup), cleared(box1)], robot, P).
